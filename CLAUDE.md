...
- `composite/` — multi-resource operations that orchestrate primitives (and the place for any `cleanup.To` lifecycle work): `clonepvc` (clone a single PVC via a `VolumeSnapshot`, or a label-selected group atomically via a `VolumeGroupSnapshot` — `ClonePVC` / `ClonePVCGroup` on one provider, sharing internal create-from-snapshot and force-bind helpers), `clonedcluster` (CNPG PITR clone with TLS — takes **no** issuer config: it creates one keyless self-signed `Issuer` internally and mints its serving + client-CA certs from it, each with a tight `CertificateRequestPolicy` via `createcrpforcertificate` **when approver-policy is detected on the cluster** — see CRP auto-detection below), `clusterusercert` (issue + CRP allow), `createcrpforcertificate` (a tight CRP derived from a given Certificate; the create→CRP→reissue dance used by both `clusterusercert` and `clonedcluster`), `drvolume` (DR PVC + CNPG ImageCatalog), `backuptoolinstance` (the in-cluster pod).

**CRP creation is auto-detected, not configured.** Whether a `CertificateRequestPolicy` is created for each issued cert is decided at runtime by `approverpolicy.ClientInterface.IsAvailable` (a memoized discovery query for the `policy.cert-manager.io/v1alpha1` API group), not by a config flag. approver-policy is an all-or-nothing cluster property: when installed it disables cert-manager's built-in CertificateRequest approver and takes over approval, so every issued cert needs a matching CRP; when absent the built-in approver auto-approves and creating a CRP would fail against a non-existent API. `clusterusercert` and `clonedcluster` gate CRP creation on this detection — there is no per-cert `enabled` knob (the only CRP option left is an optional `waitForCRPTimeout`). The dr-job ServiceAccount needs no extra RBAC for this (API discovery is granted to all authenticated identities by the default `system:discovery` binding).
