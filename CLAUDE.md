...
**Recovery target & cross-resource PITR.** `CloneCluster` is split into `CreateClusterBackup` (take the base backup — the consistency point) and `CloneClusterFromBackup` (create the recovering clone). A caller that wants the DB aligned with other DR captures takes the base backup **first** (fixing the consistency point before the filesystem/S3 captures), then creates the clone recovering forward to `T_dr` (the last non-DB capture instant). On the **plugin path**, when a `RecoveryTargetTime` is supplied the clone recovers via `recoveryTarget.targetTime`; if the source was idle between the base backup and `T_dr` there is no WAL at/after the target and recovery fails, so `CloneClusterFromBackup` **falls back to `targetImmediate`** (the consistency point — data-identical for an idle DB).
...
**Teleport/Authentik** can use either WAL-archiving path (in-tree or the plugin — the in-tree choice in the e2e is only for in-tree test coverage). They do **not** yet recover *forward* to a shared instant: their `RemoteStage` takes the base backup (in `Setup`, via the CNPG action's `CloneCluster`) before the S3 capture (in `Execute`), so the DB consistency point already precedes the non-DB capture (the safe direction — no dangling refs), but the clone is also created in `Setup`, before the S3-sync completion time `T_dr` is known. Recovering them forward to `T_dr` needs the `CloneCluster` split spread across `Setup`/`Execute` (base backup early, clone created after the S3 capture) — a `RemoteStage` restructure that is not yet implemented. Until then they recover to the consistency point on both paths. The plugin-path `targetTime`+fallback machinery in `CloneClusterFromBackup` is generic and already applies to them whenever a `RecoveryTargetTime` is supplied on the plugin path.
