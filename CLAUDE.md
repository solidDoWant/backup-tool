...
The S3 captures (Teleport audit session logs, Authentik media) are **not yet** rewound to `C`: the S3 sync action does not implement `ConsistencyPointConsumer` yet, so it runs later in the stage's `Execute` as a latest-state sync. So the event's databases share `C` and the base backups still precede the S3 capture (the safe direction — no dangling refs), but the DB-vs-S3 alignment (making the S3 action a `ConsistencyPointConsumer` and doing a version-aware point-in-time download as of `C`) is the remaining cross-resource-consistency work tracked in `docs/teleport-authentik-cross-resource-pitr.md`.
