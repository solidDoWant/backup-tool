...
type ProviderInterface interface {
	...
	CloneClusterFromBackup(ctx *contexts.Context, namespace, existingClusterName, newClusterName string, backup *apiv1.Backup, opts CloneClusterOptions) (cluster ClonedClusterInterface, err error)
}
...
func NewProvider(cucp clusterusercert.ProviderInterface, ccfp createcrpforcertificate.ProviderInterface, cmClient certmanager.ClientInterface, cnpgClient cnpg.ClientInterface, apClient approverpolicy.ClientInterface, barmanCloudClient barmancloud.ClientInterface, coreClient core.ClientInterface) *Provider {
	p := &Provider{
		cucp:              cucp,
		ccfp:              ccfp,
		cmClient:          cmClient,
