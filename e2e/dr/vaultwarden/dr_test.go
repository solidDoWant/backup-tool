...
func TestDR(t *testing.T) {
	backupReleaseName := "test-successfull-backup"
	backupCronJobName := backupReleaseName + "-dr-job"

	restoreReleaseName := "test-successfull-restore"
	...
	f1 := features.New("Successfull vaultwarden backup and recovery").
		...
		WithLabel("type", "dr").
		Setup(installBTHelmChart(backupReleaseName, namespace, "backup", "config/tests/backup.values.yaml")).
		...
		Setup(installBTHelmChart(restoreReleaseName, namespace, "restore", "config/tests/restore.values.yaml")).
		...
		Assess("new vaultwarden instance successfully deploys", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			valuesFilePath := "config/tests/restore-instance.values.yaml"
			helpOpts := append(
